@extends('master.main')

@section('content')

    @if($errors->any())

        <div class="alert alert-danger">
            <ul>
                @foreach($errors->all() as $error)

                    <li>{{ $error }}</li>

                @endforeach
            </ul>
        </div>

    @endif

    <div class="card">
        <div class="card-header">Add desk</div>
        <div class="card-body">
            <form method="post" action="{{ route('desk.store') }}" enctype="multipart/form-data">
                @csrf
                <div class="row mb-3">
                    <label class="col-sm-2 col-label-form">Description</label>
                    <div class="col-sm-10">
                        <input type="text" name="description" class="form-control" />
                    </div>
                </div>
                <div class="form-group col">
                    <label for="building_id">Choose building</label>
                    <select name="building_id"  id="building_id"  >
                        @foreach($datatablebuilding as $row)
                            <option value="" disabled selected hidden> desk</option>
                            <option value="{{$row->id}}">{{$row->description}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col">
                    <label for="level_id">Choose level</label>
                    <select name="level_id" id="level_id" >
                        @foreach($datatablelevel as $row)
                            <option value="" disabled selected hidden> desk</option>
                            <option value="{{$row->id}}">{{$row->description}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col">
                    <label for="sector_id">Choose sector</label>
                    <select name="sector_id" id="sector_id"  >
                        @foreach($datatablesector as $row)
                            <option value="" disabled selected hidden> desk</option>
                            <option value="{{$row->id}}">{{$row->description}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group col">
                    <label for="subsector_id">Choose subsector</label>
                    <select name="subsector_id" id="subsector_id"  >
                        @foreach($datatablesubsector as $row)
                            <option value="" disabled selected hidden> desk</option>
                            <option value="{{$row->id}}">{{$row->description}}</option>
                        @endforeach
                    </select>
                </div>
                <div class="row mb-4">
                    <label class="col-sm-2 col-label-form">Desk Image</label>
                    <div class="col-sm-10">
                        <input type="file" name="desk_image" />
                    </div>
                </div>
                <div class="text-center">
                    <input type="submit" class="btn btn-primary" value="Add" />
                </div>
            </form>
        </div>
    </div>

@endsection
